...
def test_database_connectivity():
    ...
    try:
        ...
        print(
            f"   ├─ Database health: {'✅ Healthy' if is_healthy else '❌ Unhealthy'}"
        )
...
def test_http_health_check():
    ...
    try:
        ...
        print(
            f"   ├─ HTTP check result: {'✅ Success' if result.is_healthy else '❌ Failed'}"
        )
...
def test_repository_integration():
    ...
    try:
        ...
        if updated_product and updated_product.quantity == 10:
            print(
                f"   ├─ ✅ Product updated successfully (new quantity: {updated_product.quantity})"
            )
        else:
