    ...
    try:
        ...
        for product in test_products:
            try:
                ...
                print(
                    f"   ✅ Added: {product.name} (qty: {product.quantity}, purchased: {product.purchased})"
                )
                added_count += 1
                time.sleep(0.1)
        ...
        if test_products_in_db:
            ...
            for product in test_products_in_db:
                ...
                print(
                    f"   {product.id} | {product.name:<15} | Qty: {product.quantity:>2} | {status}"
                )
            print("   " + "-" * 60)
        ...
        if test_products_in_db:
            ...
            print(
                f"   📝 Updated {product_to_update.name}: qty {original_quantity} → {product_to_update.quantity}"
            )
            updated_product = repo.get_product_by_id(product_to_update.id)
            if (
                updated_product
                and updated_product.quantity == product_to_update.quantity
            ):
    ...
    try:
        if os.path.exists(main_json_path):
            ...
            if len(products) < original_count:
                ...
                print(
                    f"   ✅ Removed {original_count - len(products)} test products from {main_json_path}"
                )
