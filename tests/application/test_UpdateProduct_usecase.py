...
def test_update_product(update_product_use_case, product_repository):
    ...
    product_dto = ProductDTO(id=product.id, name="Updated Product", quantity=20, purchased=True)
...
def test_update_nonexistent_product(update_product_use_case):
    product_dto = ProductDTO(id="nonexistent_id", name="Updated Product", quantity=20, purchased=True)
...
def test_update_product_invalid_quantity(update_product_use_case, product_repository):
    ...
    with pytest.raises(ValueError, match="Quantity must be positive"):
        product_dto = ProductDTO(id=product.id, name="Updated Product", quantity=-1, purchased=True)
        update_product_use_case.execute(product_dto)
